...
export default function VisualCore() {
    ...
    useEffect(() => {
        ...
        function draw() {
            ...
            const accent = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#38c4b4';
    ...
    return (
        <div className="w-full h-full bg-bg rounded-full animate-fade-in">
            <canvas
                ref={canvasRef}
                className="w-full h-full rounded-full"
                aria-label="Visual Core Animated Geometry"
            />
        </div>
    );
}
